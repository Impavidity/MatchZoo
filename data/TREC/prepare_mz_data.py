...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_dataset', action='store', default="123")
    args = parser.parse_args()
    train_dataset = args.train_dataset
    prepare = Preparation()
    srcdir = './'
    dstdir = '../TREC{}/'.format(train_dataset)

    infiles = [ os.path.join(srcdir, 'train{}.raw'.format(train_dataset)), \
                os.path.join(srcdir, 'valid{}.raw'.format(train_dataset)),\
                os.path.join(srcdir, 'test{}.raw'.format(train_dataset))]
